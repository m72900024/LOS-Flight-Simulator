...
## v2.4（2026-05-01）
- 新增 CAA AC107 雙圓考場標線：左/右圓心 (±6, 0)，內 4m / 中 6m（藍虛線）/ 外 8m，永久顯示於主場景
- 新增中央矩形測驗區邊界（橙線，沿 X 軸貫穿雙圓）
- 新增環境風力模擬：等級 0-9，含基本風 + 亂流 + 陣風（每 0.8-1.6 秒換目標方向，平滑插值）
- 新增風力對風向袋連動：風強越大，風向袋擺幅越大
- 新增 H 點智慧偵測：機體距 H 點 1m 內 + 高度 < 1.5m 時，H 字亮螢光綠輔助降落判定
- 新增右下角 2D 雷達 HUD：top-down 全域場地視圖，顯示雙圓考場、H 點、機體位置與機頭朝向
- 新增 HUD 顯示：左上加 X/Z 座標 + 風力等級指示
- 新增鍵盤快捷鍵：`[` 風力 -1，`]` 風力 +1（範圍 0-9）
- 風力影響：地面 0.3m 以下不生效（模擬地形遮蔽），高空時帶來持續漂移與姿態擾動
