...
## v2.16（2026-06-13）— 介面升級包：iPad 觸控修復 + 教室功能
🔴 iPad 觸控重大修復（實測發現）
- **雷達面板壓住右虛擬搖桿**：radar-box 與右搖桿同在右下角，觸控模式俯仰/橫滾完全摸不到 → 觸控時雷達自動上移（bottom:215px）
- **觸控 ARM/MODE/重置鈕壓住 HUD**：三顆圓鈕從 top:15px 下移到 top:112px，左右上 HUD 文字列完整可讀

教室功能
- **🔇 靜音開關**（遊戲中右側圓鈕，localStorage 持久化）：教室多台裝置同時上課防吵
- **⛶ 全螢幕按鈕**（永遠顯示，webkit 前綴相容 iPad Safari；不支援自動隱藏）
- **進關按鍵提示浮層**：依輸入模式（鍵盤/觸控/搖桿/混合）顯示對應操作，4 秒淡出或任意操作關閉
- **設定頁搖桿區預設折疊**：訊號監控/通道對應/校正/手把配置收進「🎮 搖桿設定（進階）」，手把連接自動展開——設定頁從兩頁長縮到一頁完整呈現

視覺
- **8 關獨立 accent 色**：青綠→天藍→紫→粉→橙→黃→紅橙→金（考試關壓軸），卡片頂條+星星+圖示光暈跟著走
- **草地程序化紋理**：256px 灰階明暗斑 CanvasTexture（外圈 6.25m/格、內圈同密度），LOS 平移速度感明顯提升；灰階乘法 tint 與配色面板完全相容
- **完成彈窗加「🔁 再飛一次」**：刷最佳成績不用繞回選關頁，走同一條 level-changed 重置路徑

## v2.15（2026-06-11）— 畫面健檢：HUD 重疊修正 + 定高速度目標式
UI 修正（截圖驗證）
- **場景配色面板遊戲中自動隱藏**：先前固定在左上角，蓋住 HUD 的 MODE/ARMED 整列；回選單自動恢復
- **齒輪 ⚙ 移到 top:180px**：先前與右上 HUD「最佳」重疊，也與觸控模式 ARM 鍵（同樣 top:15px right:15px）完全同位
- **設定頁加「快速開始」列**：⌨️ 鍵盤開始 / 📱 觸控開始 兩顆主按鈕放摺線上，不用捲動就能開始（先前主 CTA 在 800px 高視窗下被切掉）
- **鎖定關卡卡片調亮**：opacity 0.38→0.6、saturate 0.2→0.45，鎖住的關卡名稱看得清楚
- **考試/任務文字放大**：instruction 14→17px、exam-status 14→16px、exam-next 11→13px（教室投影幕可讀性）

飛行手感（重要行為變更）
- **定高模式爬升/下降改為速度目標式（DJI 風格）**：桿量 = 目標垂直速率，推滿 +3 m/s / 拉滿 -2 m/s
  - 先前是開環推力疊加，按住油門會無上限加速（實測 2.5 秒衝到 27m），國小生在 L1（2~3m 目標）必衝過頭
  - 修正後：鍵盤按住 W = 穩定 +1.5 m/s，2.5 秒約 3.5m，可控
- **新增「⬇️ 太高了」中央警告**：高度 > 8m（所有關卡目標都 ≤3m）紅字閃爍提示，解決飛進純藍天後零參照物迷航問題
...
- **動態 FOV**：速度越快視角越開（基準 +0~16°），相機不動只變廣度 → 速度感不破壞 LOS
- **引導空間**：lookAt 點朝速度方向前移 0.35×，飛機落在三分構圖、前進方向留白
- HUD 左上加「🎥 站位：xxx」（FPV 時隱藏）；設定面板「鍵盤控制」補上原本漏列的 C 切視角 / O 切站位

L8 模擬考國小友善化（LevelManager.js / index.html）
- **即時停懸倒數回饋**：進圈且朝向對 → 「✅ 停穩中… 再 X 秒」倒數；飄走提示「先飛到紅色圓環裡」；目標環 紅→黃→綠
- **機頭朝向檢查 + 地面金色箭頭**：每步定 heading，轉對才計時、箭頭變綠（修正舊版「叫你轉 90° 卻完全不檢查朝向」的認知陷阱）
- **下一步預告行** + 20 步文字改國小白話（箭頭 emoji + 前/後/左/右） + 放大 P1-P4 角錐與標籤
- 矩形航線 yaw 正規化為 nose-toward-travel（機頭朝行進方向），容差 ±35°；⚠️ 待對照官方 AC107-005A 確認
- cache buster 更新為 `v=20260604-l8friendly`
