...
- 50m 軟頂限制，防止飛出可視範圍
- **環境風力模擬**（v2.4）：等級 0-9 可調，含基本風 + 亂流 + 陣風，每 0.8-1.6 秒換目標方向；風向袋擺幅隨風強連動；地面 0.3m 以下遮蔽不生效

### CAA AC107 考場標線（v2.4）

對標台灣民航局**遙控無人機基本級術科測驗**場地佈置，永久顯示於主場景：

- **雙圓設計**：左右圓心 (±6, 0)，內 4m / 中 6m（藍虛線）/ 外 8m
- **中央測驗區**：橙色矩形邊界沿 X 軸貫穿雙圓
- **H 點智慧偵測**：機體距 H 點 1m 內 + 高度 < 1.5m 時，H 字自動亮**螢光綠**輔助降落判定

讓你的訓練場地與真實考場視覺一致。

### 全域 HUD 系統（v2.4 強化）

- **左上**：油門 / 飛行模式 / 解鎖狀態 / **X-Z 座標** / **🌬 風力等級**
- **中央**：關卡標題 + 任務指示 + 進度條
- **右上**：用時 / 最佳成績 / 高度 / 距下個目標 / 機頭方位
- **右下**：**2D 雷達**（top-down 全域場地視圖，顯示雙圓考場、H 點、機體位置與機頭朝向箭頭）
...
| `R` | 重置 | 重置無人機位置與姿態 |
| `K` | 切換輸入 | 鍵盤 ↔ 搖桿模式切換 |
| `[` / `]` | 風力 -1 / +1 | 環境風力等級 0-9（v2.4），按住 ] 持續加風 |
...
> 受 [tamago797/Drone-Simulator-](https://github.com/tamago797/Drone-Simulator-) 啟發，借鑒其考場 1:1 還原、智慧 H 點、風力模擬三項設計，獨立重新實作並整合進現有 LOS 模組化架構。

- **CAA AC107 雙圓考場標線**：左右圓心 (±6, 0)，內 4m / 中 6m（藍虛線）/ 外 8m，永久顯示於主場景；中央矩形測驗區邊界（橙線，沿 X 軸貫穿雙圓）
- **環境風力模擬 0-9 級**：基本風 + 亂流 + 陣風，每 0.8-1.6 秒換目標方向，平滑插值；風向袋擺幅跟風強連動；地面 0.3m 以下遮蔽不生效（模擬地形）
- **H 點智慧偵測**：機體距 H 點 1m 內 + 高度 < 1.5m 時，H 字自動亮螢光綠輔助降落判定
- **2D 雷達 HUD**：右下角 140px 全域場地視圖，繪製雙圓考場、H 點、機體位置與機頭朝向箭頭
- **HUD 強化**：左上加 X/Z 座標 + 🌬 風力等級指示（風 4 級以上轉紅）
- **新增鍵盤快捷**：`[` 風力 -1，`]` 風力 +1（範圍 0-9）
...
- **[tamago797/Drone-Simulator-](https://github.com/tamago797/Drone-Simulator-)** — v2.4 的雙圓考場標線、H 點智慧偵測、風力模擬三項設計概念來自此專案啟發。所有相關程式碼為獨立重新實作（未複製其原始碼），整合進本專案模組化架構。
- **台灣交通部民航局** — [AC107-005 遙控無人機術科測驗規範](https://www.caa.gov.tw/article.aspx?a=2621&lang=1) 為 L8 模擬考試與場地佈置的設計依據。
